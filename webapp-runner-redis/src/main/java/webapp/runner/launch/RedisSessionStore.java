...
public class RedisSessionStore extends SessionStore {
  ...
  public void configureSessionStore(CommandLineParams commandLineParams, Context ctx) {
    ...
    if (System.getenv("REDIS_URL") == null
        && System.getenv("REDISTOGO_URL") == null
        && System.getenv("REDISCLOUD_URL") == null) {
      System.out.println(
          "WARNING: using redis session store, but the required environment variable isn't set.");
      System.out.println(
          "Redis session store is configured with REDIS_URL, REDISTOGO_URL or REDISCLOUD_URL");
    } else {
      if (System.getenv("REDIS_URL") != null) {
      ...
      SingleServerConfig serverConfig =
          config
              ...
              .setSslEnableEndpointIdentification(
                  commandLineParams.sessionStoreSslEndpointIdentification)
